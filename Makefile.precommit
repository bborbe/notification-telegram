...
addlicense:
	...
	@for f in $$(find . -name "*.go" -not -path './vendor/*'); do \
	  if head -1 $$f | grep -q "Code generated by counterfeiter"; then \
	    { head -1 $$f; echo "// Copyright (c) $$(date +'%Y') Benjamin Borbe All rights reserved."; echo "// Use of this source code is governed by a BSD-style"; echo "// license that can be found in the LICENSE file."; tail -n +2 $$f; } > $$f.tmp && mv $$f.tmp $$f; \
	  fi; \
	done
